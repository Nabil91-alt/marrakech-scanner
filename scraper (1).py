#!/usr/bin/env python3
"""
MARRAKECH IMMOBILIEN-SCRAPER v3
...
def _mubawab_detail(listing, session):
    ...
    # Kontaktdaten
    for sel in ["[class*='phone']","[class*='tel']","[class*='contact']","a[href^='tel:']"]:
        el = soup.select_one(sel)
        if el:
            txt = el.get_text(strip=True)
            if re.search(r'\d{8,}', txt.replace(" ","")):
                listing.contact_phone = listing.contact_phone or txt
            href = el.get("href","")
            if href.startswith("tel:"):
                listing.contact_phone = listing.contact_phone or href.replace("tel:","")
    for sel in ["[class*='agent']","[class*='seller']","[class*='name']"]:
        el = soup.select_one(sel)
        if el and not listing.contact_name:
            name = el.get_text(strip=True)
            if 2 < len(name) < 50 and not re.search(r'\d{5}', name):
...
def _avito_detail(listing, session):
    ...
    for script in soup.find_all("script"):
        ...
        if '"phone"' in text or '"phoneNumber"' in text:
            for m in re.findall(r'"(?:phone|phoneNumber)"\s*:\s*"([^"]+)"', text):
                if not listing.contact_phone and len(m) >= 8:
                    listing.contact_phone = m
    if not listing.price_mad:
    ...
    if not listing.description:
        for sel in ["[class*='description']","[class*='body']"]:
            ...
            if el: listing.description = el.get_text(" ", strip=True); break
    # Kontakt
    for sel in ["[class*='phone']","a[href^='tel:']","[class*='contact'] a","button[class*='phone']"]:
        el = soup.select_one(sel)
...
def _sarouty_detail(listing, session):
    ...
    if not listing.description:
        for sel in ["[class*='description']","[class*='text']"]:
            el = soup.select_one(sel)
            if el: listing.description = el.get_text(" ", strip=True); break
    for sel in ["[class*='phone']","a[href^='tel:']","[class*='contact'] a"]:
        el = soup.select_one(sel)
        if el and not listing.contact_phone:
            href = el.get("href","")
            if href.startswith("tel:"): listing.contact_phone = href.replace("tel:","")
                ...
                txt = el.get_text(strip=True)
                if re.search(r'\d{8,}', txt.replace(" ","")): listing.contact_phone = txt
    for sel in ["[class*='agent']","[class*='name']"]:
        el = soup.select_one(sel)
        if el and not listing.contact_name:
            name = el.get_text(strip=True)
...
def main():
    ...
    print(f"\n  MARRAKECH SCRAPER v3 | {args.portal.upper()} | {args.pages} Seiten\n")
    ...
    for i, l in enumerate(passed[:20], 1):
        ...
        print(f"  {i}. {p} MAD | {a} | {r} | {l.neighborhood or '?'} | {l.source}{ph}")
        print(f"     {l.title[:70]}")
        print(f"     {l.url}")
